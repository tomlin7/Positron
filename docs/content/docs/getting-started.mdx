...
<Callout type="info">
The `[full]` extra includes JavaScript support via PythonMonkey, which is required for React apps. The `TkinterWeb/` folder in the repo is for reference only. Positron uses the TkinterWeb package from pip.
</Callout>
