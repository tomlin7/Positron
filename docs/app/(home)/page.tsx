      ...
      <div className="mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="grid gap-x-12 gap-y-10 sm:grid-cols-2">
          ...
          <div className="space-y-8">
            <div>
              <h3 className="mb-3 text-[10px] uppercase tracking-[0.3em] text-zinc-500">01. Finished</h3>
              <ul className="space-y-1 text-sm font-mono text-zinc-400 italic">
                {completed.map(i => <li key={i}>/ {i}</li>)}
              </ul>
            </div>

            <div>
              <h3 className="mb-3 text-[10px] uppercase tracking-[0.3em] text-white">02. Building</h3>
                ...
                {active.map(i => (
                  <div key={i.name}>
                    ...
                    <div className="h-[1px] w-full bg-zinc-900 overflow-hidden">
                      <div className="h-full bg-white transition-all duration-1000" style={{ width: i.p }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Section: Roadmap */}
          <div>
            <h3 className="mb-3 text-[10px] uppercase tracking-[0.3em] text-zinc-600">03. Pipeline</h3>
            <div className="grid grid-cols-1 gap-1 text-[12px] font-mono text-zinc-700">
              {next.map(i => <span key={i}>+ {i}</span>)}
            </div>
          </div>
        </div>
